...
class Retriever(object):  #下载web页面。解析文档中的链接，加入到链接列表
    ...
    def get_file(self, url, default='index.html'):  #根据url地址转换为本地地址，用于存储
        parsed = urlparse.urlparse(url)   #返回6元组
    ...
    def download(self):  #根据url下载文件
        try:
            retval = myurllib2.urlretrieve(self.url, self.file)  #下载url文件到指定本地文件
    ...
    def parse_links(self):   #解析刚下载下来的页面链接
        ...
        data = f.read()
        f.close()
        parser = htmllib.HTMLParser(formatter.AbstractFormatter(   #AbstractFormatter用来解析数据
            formatter.DumbWriter(cStringIO.StringIO())))  #DumbWriter用来输出内容，cStringIO保障不输出到标准输出（最好输出到文件）
        parser.feed(data)
        parser.close()
    ...
    def get_page(self, url, media=False):  #下载页面，解析页面，调整链接列表
        ...
        for link in r.parse_links():  #解析文件，获取文件中的连接列表
            ...
            if not link.startswith('http://'):    #如果连接是相对路径
                link = urlparse.urljoin(url, link)  #相对url路径，转变为绝对url路径
            print('发现网页：'+link)
